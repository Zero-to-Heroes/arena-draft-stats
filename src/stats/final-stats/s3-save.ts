...
export const saveDraftStats = async (stat: DraftStatsByContext, timePeriod: TimePeriod, s3: S3): Promise<void> => {
	const gzippedResult = gzipSync(JSON.stringify(stat));
	const destination = `${ARENA_STATS_KEY_PREFIX}/${timePeriod}/${stat.context.toLowerCase()}.gz.json`;
